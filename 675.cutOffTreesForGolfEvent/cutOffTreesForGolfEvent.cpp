...
public:
    int cutOffTree(vector<vector<int>>& forest) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (forest[i][j] != 0)
                    trees.push_back(Node(forest[i][j], i, j));
            }
        }
        if (trees.size() == 0)
            return -1;
    ...
    int bfs(vector<vector<int>>& forest, int start_x, int start_y, int dest_x, int dest_y,
            ...
            for (int i = 0; i < q_size; i++) {
                ...
                for (auto& dir : dirs) {
                    ...
                    if (next_x < 0 || next_x >= m || next_y < 0 || next_y >= n)
                        continue;
                    if (forest[next_x][next_y] == 0 || visited[next_x][next_y])
                        continue;
