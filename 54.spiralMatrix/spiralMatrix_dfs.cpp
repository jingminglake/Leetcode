...
public:
    ...
    void dfs(vector<vector<int>>& matrix, vector<vector<bool>>& visited, vector<pair<int, int>>& dirs, int index,
             vector<int>& res, int i, int j) {
        if (i < 0 || i >= matrix.size() || j < 0 || j >= matrix[0].size() || visited[i][j])
            return;
        visited[i][j] = true;
        res.push_back(matrix[i][j]);
        int next_i = i + dirs[index % 4].first, next_j = j + dirs[index % 4].second;
        while (next_i >= 0 && next_i < matrix.size() && next_j >= 0 && next_j < matrix[0].size() &&
