...
public:
    ...
    bool findFromHere(vector<vector<char>>& board, int i, int j, string word, int depth) {
        ...
        if (board[i][j] != word[depth])
            return false;
        board[i][j] = '*';
        bool find = findFromHere(board, i + 1, j, word, depth + 1) || findFromHere(board, i - 1, j, word, depth + 1) ||
                    findFromHere(board, i, j + 1, word, depth + 1) || findFromHere(board, i, j - 1, word, depth + 1);
