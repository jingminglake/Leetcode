...
class User {
public:
    User(int id) : userId(id) { tweetList = nullptr; }
    ~User() {
        while (tweetList) {
    ...
    unordered_set<int>& getFollowed() { return followed; }
...
class Compare {
public:
    bool operator()(const Tweet* t1, const Tweet* t2) const { return t1->getTweetTime() < t2->getTweetTime(); }
};
...
public:
    ...
    void unfollow(int followerId, int followeeId) {
        if (followerId == followeeId || userMap.find(followerId) == userMap.end() ||
